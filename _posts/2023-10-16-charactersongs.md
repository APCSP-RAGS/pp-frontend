...
.toggle-lyrics-button {
    background-color: #000; /* Match background color with the hacker-theme */
    color: #0f0; /* Match font color with the hacker-theme */
    border: none;
    padding: 8px;
    border-radius: 10px;
    cursor: pointer;
        ...
        .then(data => {
          ...
          data.forEach(Song => {
            ...
            row.innerHTML = `
                ...
                <td>${Song.genre}</td>
                <td class="lyrics">${Song.lyrics}</td>
                <td><button class="toggle-lyrics-button" onclick="toggleLyrics(this.parentNode.parentNode)">Toggle Lyrics</button></td>
                `;
...
>>>>>>> 040a4e3 (added jesse better picture)
